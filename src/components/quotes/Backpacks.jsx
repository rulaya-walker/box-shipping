...
const Backpacks = ({ getQuantity, updateQuantity, setShowAddBoxForm, onItemIdsChange,toCountry }) => {
  ...
  useEffect(() => {
    dispatch(fetchProductsByCategory('Backpacks'));
  }, [dispatch]);
    ...
    return (
      <div>
        ...
        <div className="text-center py-8">
          ...
          <button 
            onClick={() => dispatch(fetchProductsByCategory('Backpacks'))}
    ...
    <div>
      ...
      <div className="space-y-4">
        {productsByCategory && productsByCategory.length > 0 ? (
              ...
              <div key={product._id || product.id} className="border rounded-lg p-4 hover:shadow-lg transition-shadow">
                <div className="flex items-center gap-6">
                  <div className="w-24 h-24 bg-gray-100 rounded-lg flex items-center justify-center flex-shrink-0">
                    <img
                      src={product.image?.url || backpacks}
                      alt={product.name}
                      className="w-full h-full object-cover rounded-lg"
                      onError={(e) => {
                        e.target.src = backpacks;
                      }}
                    />
                  </div>
          ...
          <div className="text-center py-8">
            <p className="text-gray-600">No backpacks products available at the moment.</p>
          </div>
        )}
        ...
        {/* <div className="border rounded-lg p-4 hover:shadow-lg transition-shadow">
          <div className="flex items-center gap-6">
            ...
            <div className="flex-1">
              <h4 className="font-semibold mb-2">Add a backpack</h4>
              <p className="text-gray-600 text-sm mb-4">
